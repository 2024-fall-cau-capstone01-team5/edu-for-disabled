...
class _Scenario_c_5_leftState extends State<Scenario_c_5_left> {
  ...
  Future<void> _playWelcomeTTS() async {
    ...
    await Provider.of<Scenario_Manager>(context, listen: false)
        .updateSubtitle("편의점 진열대에 물건들이 진열되어 있네요.");

    await tts.TextToSpeech("편의점 진열대에 물건들이 진열되어 있네요. ", "ko-KR-Wavenet-D");
    await tts.player.onPlayerComplete.first;

    await Provider.of<Scenario_Manager>(context, listen: false)
        .updateSubtitle("여러분들이 선택했던 물건이 기억이 나시나요?\n"
            "그 물건을 진열대에서 직접 손가락으로 눌러보세요.");
    await tts.TextToSpeech(
        "여러분들이 선택했던 물건이 기억이 나시나요? "
            "그 물건을 진열대에서 직접 손가락으로 눌러보세요.",
        "ko-KR-Wavenet-D");
    await tts.player.onPlayerComplete.first;

    await Provider.of<Scenario_Manager>(context, listen: false)
        .updateSubtitle("만약 기억이 나지 않아도 괜찮아요.\n "
...
class Scenario_c_5_right extends StatefulWidget {
  ...
  @override
  State<Scenario_c_5_right> createState() => _Scenario_c_5_rightState();
}
